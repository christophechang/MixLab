...
@respx.mock
async def test_revise_concepts_accepted_repair_preserves_concept_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """conceptId unification (#89): an accepted repair must keep the pre-revision
    concept_id — it is the join key between the history record and summary.json, and
    must never be regenerated by the revision pass."""
    from mixlab.llm import revise_concepts, validate_stage2_output
    ...
    concepts, _report, _final_warnings = await revise_concepts(
        [original],
        "PROSE REPORT\n\n---\n\nMain brain: test-model",
        warnings,
        [canvas],
    ...
    assert concepts[0].track_ids == ["1", "2", "9", "4", "5", "6", "7", "8"]
    assert concepts[0].concept_id == "11111111-1111-1111-1111-111111111111"
