...
async def run(
    ...
    # operator picked one direction, so enumerating others would dilute the run (and risk
    # a near-duplicate canvas). Spec failures are fatal: silently delivering a run without
    # the clicked direction is exactly the bug this flag exists to fix.
    pinned_canvas: MixCanvas | None = None
    if direction_spec:
        try:
            pinned_canvas = pinned_canvas_from_spec(direction_spec, tracks_by_id)
        except DirectionSpecError as exc:
            print(str(exc), file=sys.stderr)
            sys.exit(1)
        print(f"Pinned direction: {pinned_canvas.direction_type} — {pinned_canvas.source_concept.title}")
        if directions != "mixed":
            print(
                f"--directions {directions} superseded by --direction-spec — the pinned direction "
                "replaces the enumerated-directions slot.",
                file=sys.stderr,
            )
    ...
    if pinned_canvas is not None and not any(
        _match_canvas_for_concept(c, [pinned_canvas]) is not None for c in all_concepts
    ):
        print(
            "WARNING: pinned direction produced no concept — Stage 2 skipped the pinned canvas "
            f"({pinned_canvas.source_concept.title}).",
            file=sys.stderr,
        )
    html = render_html_report(
        all_concepts,
...
""",
    ...
    parser.add_argument(
        ...
        dest="direction_spec",
        help=(
            "JSON-serialised library-map direction entry to pin for this run (machine-generated by "
            "mixlab-web's 'Run this direction'). Materialises that exact direction — its tracks, brief "
            "and type — as a mandatory canvas, replacing the enumerated --directions slot. Genre mode "
            "only; ignored in playlist mode."
        ),
    )
    parser.add_argument(
        "--risk",
    ...
    asyncio.run(
        run(
            ...
            direction_spec=args.direction_spec,
