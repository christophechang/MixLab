...
Alongside that named vocabulary sits an unnamed one. The same pool is also mined for statistically dense predicate conjunctions no builder has a word for — a label that happens to cluster inside one five-year era, a tag that only ever shows up in one tempo pocket:

- **found_1**, **found_2**, **found_3** — a mined conjunction that is materially denser than chance (lift ≥ 1.3 over ≥ 15 shared tracks), titled from its human-legible half only. Up to three per pool; a run ships at most one, so a discovery adds to the day's variety rather than taking it over

Each proposed direction — named or mined — is feasibility-scored on three terms, each of which actually spreads across a real collection: **freshness** (how recently the shipped tracks were added, as a rank percentile within the pool), **identity** (how strong the defining idea is — pole balance, a label's density relative to the wider collection, a mined pair's lift), and **distinctiveness** (how little the shipped tracks overlap every other direction surviving in that pool). Pool size and BPM-path viability are pass/fail gates rather than scored terms — every candidate that gets built passes both, so scoring them told you nothing — which means briefs that cannot actually be mixed are dropped outright rather than merely marked down. Feasible directions are then seed-rotated: the same seed reproduces the same picks, but different days (the seed defaults to the date, reproducible via `--stage1-seed`) surface different angles while the strongest directions still appear often. Each surviving direction becomes a Mix Canvas carrying a DIRECTION BRIEF that Stage 2 must honour as the concept's thesis.
...
In `mixed`, the two families are composed rather than concatenated: directions are picked first, then classic canvases are over-selected and filtered against them, so a classic canvas that duplicates a direction's pool is dropped instead of reaching Stage 2 as the same crate a second time. The field is topped back up from the next-best classic canvases, and each drop is named on stderr.
...
- Concept titles are held to **mix-title register** — a self-contained noun phrase that could be printed on a sleeve and survives being said out loud with no context. Register is stated as a hard constraint ahead of technique, and the seed-rotated naming lenses (three per run, so name character varies day to day) all land on that shape: a lens varies *how* a title is built, never *what shape* it comes out as
...
- Post-Stage-2 validation is warn-only. Strong-tier checks (always fire): track IDs not found in the library, denylist or played-track violations, Camelot jumps greater than 4, BPM jumps greater than 15, artist repeats of 3 or more, opener/closer absent in expected positions, bridge/wildcard tracks used without a justified `risk_type`, and wildcard tracks used outside the canvas's concept-anchor list. Soft-tier checks (softened by genre family and `arc_type`): no peak in sequence, no wind-down before closer, three-or-more consecutive same-role-family tracks, all tracks high-energy, cross-concept track overlap above 50%, generic `[Adjective][Noun]` concept titles, and titles that fall out of mix-title register (a first-person aside, a phrase that stops mid-thought, trailing punctuation). Warnings appear under **⚠ Validation Notes** and never abort the run.
- One check is *not* warn-only: cross-concept track overlap of 90% or above. Two concepts that similar are the same set wearing two names, so the later one is dropped before the report pass rather than flagged, and the report records what went and what it duplicated. Genre mode only — playlist mode's three variants are deliberate readings of one completion with a single winner picked downstream.
